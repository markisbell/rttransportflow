...
```jsonc
{
  ...
  "device_commands": {
    ...
    "ocgt_2":  {"breaker": "close", "black_start": true}
                 // black_start (sync plants, with breaker "close" only):
                 // the start BYPASSES the sick-island hold and the unit
                 // re-energizes its blacked-out island at 50 Hz onto the
                 // unloaded system, syncing at house load (ledger 34);
                 // demand returns only via zone_commands.restore_load
                 // through the 5-min-healthy gate
  },
...
```jsonc
{
  ...
  "islands": {"0": {"f_hz": 49.982, "rocof_hz_s": -0.012, "f_min": 49.90,
                     ...
                     "angle_spread_rad": 0.06}},
                     // h_sys_s is derived: h_sys_s = e_k_mj / s_online_mva
                     // (fixtures assert the identity). angle_spread_rad is the
                     // rotor-angle spread over the island's online sync
                     // machines (phasor observer, ledger 56): ~0 when steady,
                     // grows toward pi as a pocket loses synchronism.
  ...
  "devices": {"plant_9": {"p_mw": 478.9, "q_mvar": 120.2, "soc": null,
               ...
               "delta_rad": 0.21, "slip_hz": 0.0}},
               // delta_rad/slip_hz on every SYNC device (phasor observer,
               // ledger 56): the rotor angle in the island COI frame and the
               // speed slip from that COI (0 steady, non-zero while swinging).
               // Additive to contract 2.0 — the game reads them null-tolerantly.
